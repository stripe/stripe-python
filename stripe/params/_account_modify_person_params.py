...
class AccountModifyPersonParams(RequestOptions):
    ...
    """
    birth_address: NotRequired["AccountModifyPersonParamsBirthAddress"]
    """
...
class AccountModifyPersonParamsBirthAddress(TypedDict):
    city: NotRequired[str]
    """
    City, district, suburb, town, or village.
    """
...
class AccountModifyPersonParamsSelfReportedIncome(TypedDict):
    amount: int
    """
...
class AccountModifyPersonParamsSelfReportedMonthlyHousingPayment(TypedDict):
    amount: int
    """
    The amount in the minor currency unit (for example, cents for USD).
    """
