...
class TestTransferReversal(object):
    def construct_resource(self):
        ...
        return stripe.TransferReversal.construct_from(
            reversal_dict, stripe.api_key
        )
    ...
    def test_is_not_modifiable(self):
        with pytest.raises(NotImplementedError):
            stripe.TransferReversal.modify(
                TEST_RESOURCE_ID, metadata={"key": "value"}
            )

    def test_is_not_retrievable(self):
        with pytest.raises(NotImplementedError):
            stripe.TransferReversal.retrieve(TEST_RESOURCE_ID)
