...
-   [#1159](https://github.com/stripe/stripe-python/pull/1159) Fix **getattr** to raise AttributeError rather than returning None. This fixes a regression in 7.8.0 that caused `stripe.checkout`/`stripe.issuing` etc. to return `None`.
...
-   [#1142](https://github.com/stripe/stripe-python/pull/1142) Move resource type exports to stripe.\_\_\_
...
-   [#422](https://github.com/stripe/stripe-python/pull/422) Add `user_mesage` to `StripeError` for a way in Python 3 to avoid the "Request req\_...:" string normally appended to error messages
